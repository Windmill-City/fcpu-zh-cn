...
local function splice(array, start, stop, replacement)
    if replacement then
        ...
        for _, v in ipairs(replacement) do table.insert(array, start, v) end
        return array
    else
        local res = {}
        for i = start, stop do table.insert(res, array[i]) end
        return res
    end
end
...
local function tokenize_html(html)
    ...
    while true do
        ...
        if start ~= pos then
            table.insert(tokens,
                         {type = "text", text = html:sub(pos, start - 1)})
        end
...
local function is_protected(s) return PD.blocks[s] end
...
local function lists(array, sublist)
    ...
    local function find_list(array, sublist)
        local function find_list_start(array, sublist)
            ...
            if sublist then
                ...
                for i = 1, #array - 1 do
                    if array[i].type == "blank" and array[i + 1].type ==
                        "list_item" then return i + 1 end
                end
            end
            return nil
        end
        local function find_list_end(array, start)
            ...
            for i = start, #array - 1 do
                if array[i].type == "blank" and array[i + 1].type ~= "list_item" and
                    array[i + 1].type ~= "indented" and array[i + 1].type ~=
                    "blank" then
                    pos = i - 1
                    break
                end
    ...
    while true do
        ...
        local info = {line = text, type = "raw", html = text}
...
local function add_escape(text)
    if not escape_table[text] then escape_table[text] = hash(text) end
...
function encode_code(s)
    ...
    s = s:gsub("<", "&lt;")
    s = s:gsub(">", "&gt;")
    for k, v in pairs(escape_table) do s = s:gsub("%" .. k, v) end
    return s
end
...
local function anchors(text)
    local function reference_link(text, id)
        ...
        return
            add_escape("<a href=\"" .. url .. "\"" .. title .. ">") .. text ..
                add_escape("</a>")
    end
    ...
    local function inline_link(text, link)
        ...
        url = encode_alt(url)
        if title then
            return add_escape("<a href=\"" .. url .. "\" title=\"" .. title ..
                                  "\">") .. text .. "</a>"
        else
            return add_escape("<a href=\"" .. url .. "\">") .. text ..
                       add_escape("</a>")
        end
    end
...
if arg and arg[0]:find("md2frt%.lua$") then
