...
Factorio 线缆可同时承载数百个信号, 但信号本身没有顺序, 索引随机。为便于查看, GUI 面板会按降序排序(电线杆与信息提示均如此)

原版运算器由 C++ 实现, 处理大量信号很快; 而 Lua 处理同等规模的信号开销极大、性能堪忧, 这对所有 Factorio 模组都一样

为此, 程控运算器将程序中的 SIMD 指令(`x*`)部分编译为原版运算器组成的隐形电路, 并逐行协调执行。这部分称为协处理器, 性能远胜 Lua 方案

寄存器以普通变量实现, 始终保留索引; 而 SIMD 指令使用的内存同样由原版运算器承载
